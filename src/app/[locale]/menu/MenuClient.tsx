    ...
    <div className="flex flex-col gap-[3vw] max-md:gap-[8vw]">
      ...
      {blocks.map(({ category, items }, bi) => (
        <section key={category.id} id={`cat-${category.id}`} data-testid="menu-category" className="scroll-mt-[10vw] max-md:scroll-mt-[26vw] flex flex-col gap-[1.5vw] max-md:gap-[5vw]">
          ...
          {/* Kural 45 + ölçüm 2026-09-18: ilk boyama HER ZAMAN filtresizdir (filtre
              hydrate sonrası uygulanır) → sunucunun yaydığı preload ilk boyamada
              doğru karta işaret eder. Ölçüm: priority AÇIK mobil 2536 ms / masaüstü 476 ms,
              KAPALI 2752 / 524 ms → açık kalıyor. */}
          <ProductGrid products={items} onSelect={open} priorityFirst={bi === 0 && filter === null} />
        </section>
      ))}
